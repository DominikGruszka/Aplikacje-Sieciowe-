...
{strip}
	<tr>
		<td>{$p["surname"]}</td>
		<td>{$p["kwota_kredytu"]}</td>
		<td>{$p["lata_splaty"]}</td>
                <td>{$p["oprocentowanie"]}</td>
		<td>
			<a class="button-small pure-button button-secondary" href="{$conf->action_url}personEdit&id_kredyty={$p['id_kredyty']}">Edytuj</a>
			&nbsp;
			<a class="button-small pure-button button-warning" href="{$conf->action_url}personDelete&id_kredyty={$p['id_kredyty']}">Usuń</a>
		</td>
	</tr>
{/strip}
{/foreach}
